...
shinyServer(function(input, output, session){ 
  ...
  observe({  # Representa o primeiro critério de atualização do banco de dados (no total são 3)
    ...
    #dados <<- read_excel("C:\\Users\\ferna\\OneDrive\\Área de Trabalho\\Sistema\\Base (2).xlsx", sheet=1, col_names=TRUE)
    dadosP <<- read_excel("C:\\Users\\ferna\\OneDrive\\Área de Trabalho\\Sistema\\Base (2).xlsx", sheet=2, col_names=TRUE)
    Date <<- read_excel("C:\\Users\\ferna\\OneDrive\\Área de Trabalho\\Sistema\\Base (2).xlsx", sheet=3, col_names=TRUE)
    ...
    observeEvent(input$dashboard5, { # saídas referentes ao DASHBOARD e Pré-Requisitos
      ...
      if(input$dashboard1 != "Todos os meses" & input$dashboard2 != "Todos os anos" & input$dashboard4 == "Ambos"){
        ...
        dadosData4$Classificação <- as.character(dadosData4$Classificação)
        ...
        dadosData4$Classificação <- as.numeric(dadosData4$Classificação)
        dadosData4$Status <- as.numeric(dadosData4$Status)
        dadosData4$Soma <- as.numeric(dadosData4$Soma)
        
        fit <- survfit(Surv(Soma, Classificação) ~ Status, data = dadosData4)
        ...
        output$histogram2 <- renderPlot({ # Imagem 2 - Dashboard
          ...
          fit2 <- survfit(Surv(Soma, Classificação) ~ Status, data = dadosData4)
